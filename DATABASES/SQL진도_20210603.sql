...
FROM dept
WHERE loc = 'NEW YORK';
-- DUAL 가상테이블이름. 테이블이 없는 내용을 select 할때
SELECT 3+5 as "3더하기8은" from dual;
--레코드 (row) : 컬럼(필드field)들도 구성
SELECT concat (count(*),'명') as "연봉이 2000인 직원" FROM emp WHERE sal > 1000;
...
-- NVL2(필드명,널이 아닐때 100, 널일때 0)NVL(필드명,널일때0)
-- 오라클은 표준쿼리 x, ANSI쿼리 표준O입니다.
SELECT 
CASE WHEN comm is null THEN 0
WHEN comm = 0 THEN 100
WHEN comm > 0 THEN comm
END AS "CASE출력문"
,DECODE(comm is null THEN
,NVL2(comm,100,0)
...
-- 위 정렬에서 1등만 구할 limit는 Mysql(마리아DB)의 명령어.
-- Mysql(마리아DB)있는 AI(AutoIncrement)자동증가값 명령 오라클X
-- 중복레코드(row)를 제거하는 명령 distinct
