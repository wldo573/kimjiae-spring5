...
-- 제약조선(constraint) 이 자동생성, Index(테이블)도 자동생성(검색시중요)
...
--DEPT 테이블의 deptno 숫자2자리때문에 에러->4자리 크기를 변경
...
--댓글개수 구할때,
--카티사안프로덕트 조안(합집합-게시물10개.댓글100=110개~)
--(인너)조인(교집합)을 제일 많이사용
--아래조인방식 Oracle방식(아래)
...
WHERE e.ename ='SCOTT';
--조인과 그룹을 이용해서 댓글카운터가 들어가는 조인그룹을 이용해 댓글카운터도 출력하는 개시판리스트만들기
SELECT bod.bno,title,conut(*) as reply_count
...
ORDER BY bod.bno;
--9장 패스(레포트용 함수사용)
--8장 함수(count,uppermlower,to_char,round...)그룹함수
...
SELECT deptno, SUM(sal), ename, sal FROM emp
GROUP BY deptno,ename,sal
ORDER BY SUM(sal) DESC;
--라우드 함수(반올림)소수점기준. round(10.05,2)소수점2째반올림
SELECT ename, round(sal,-3) FROM emp;--레크드가 여러개
...
--위 AVG함수를 Where조건에 사용 못할때 서브쿼리를 이용합니다.
SELECT MAX(sal)
,MIN(sal) 
