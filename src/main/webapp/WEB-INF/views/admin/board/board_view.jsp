              ...
              <div class="form-group">
                ...
                <c:forEach begin="0" end="1" var="idx">
	                ...
	                <div class="input-group">
	                  ...
	                  <!-- 첨부파일을  URL로 직접접근하지 못하기 때문에  다운로드 전용 메서드 생성-->
	                    <a href="/download?save_file_name=${boardVO_save_file_names[idx]}&real_file_name=${boardVO.real_file_names[idx]}" >
	                    ${boardVO.real_file_names[idx]}
	                    </a>
	                    <!-- jstl에서 변수 설정사용하기 fn.split('데이터','분할기준값') 목적: 확장자를 이용해서 이미지 미리보기를 할건지 결정 img태그사용
	                    	String[] fileNameArray = String.split('변수값','분할기준값');
	                    -->
	                    <c:set var="fileNameArray" value="${fn:split(boardVO.save_file_names[idx],'.')}"/>
	                  	<c:set value="extName" value="${fileNameArray[fn:length(fileNameArray)}"/>
	                  </div>
	                </div>
	                </c:if>
                </c:forEach>
