	...
	<div class="bodytext_area box_inner">
		...
		<form method="POST" name="join_form" action="/member/mypage" class="appForm">
			<fieldset>
				...
				<button type="button" class="btn_baseColor" id="btn_leave">회원탈퇴</button>
				</p>	
			</fieldset>
		</form>
		<!-- //폼영역 -->
...
$(document).ready(function(){
	$("#btn_leave").click(function(){
		if(confirm('정말로 탈퇴하시겠습니까?')){
			...
			$("option:eq(0)","select[name='enabled']").val("false");//Set input태그
			//$("select[name='enabled']").html("<option value='flase'>탈퇴</option>");//select Set
			//alert($("select[name='enabled']").val());//Get
			//위값을 false, 0 둘중 1개로 변경 후 submit예정
