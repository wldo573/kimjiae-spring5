 ...
 * 디스패처 서블렛 클래스는 톰캣이 실행될때(web.xml) 제일 먼저 실행되는 클래스, 그래서, 게이트웨이라고 합니다
 * 디스패처 서블릿 실행될때, 컨트롤러의 Request매핑경로를 재 등록합니다.
...
public class AdminController {
	...
	public String board_list(@ModelAttribute("pageVO")PageVO pageVO,Model model) throws Exception{//목록폼1
		...
		pageVO.setPerPageNum(5);//1페이징 UI하단에서 보여줄 페이징 번호 크기
	...
	public String updateBoardType(BoardTypeVO boardTypeVO) throws Exception{//수정처리1
		...
		return "redirect:/admin/bbs_type/bbs_type_update?board_type="+boardTypeVO.getBoard_type();//.jsp생략
	}
