...
<<<<<<< HEAD
	...
	public String  board_view(@RequestParam("bno")Integer bno,@ModelAttribute ("pageVO")PageVO pageVO, Model model) throws Exception{
		...
		//attach 테이블안의 해당 bno게시물의 첨부파일 이름 파싱해서 jsp로 보내주는 과정(아래)
		int cnt = 0;
		...
		//위 for은 세로데이터(다수레코드)를 가로데이터(1개의 레코드이면, 배열)에 담아서 1개의 레코드 boardVO로 담드는게 목적
		...
		//업로드한 파일이 이미지인지 아닌지 확인하는 용도의 데이터 입니다.(이미지일때 미리보기 img태그를 사용하기위해서)
	...
	public String board_list(@ModelAttribute("pageVO")PageVO pageVO,Model model) throws Exception{//목록폼1
=======
