...
public class CommonUtil {
	...
	//첨부파일 개별삭제 Ajax로 받아서 처리,@ResponseBody사용
		...
		@ResponseBody
		public String file_delete(@RequestParam("save_file_name")String save_file_name){//Ajax는 예외처리를 스프링에 던지지 않고,try~caych문으로 처리
	...
	//다운로드 처리 같은 페이지에서 결과값만 반화받는 @ResponseBody 사용
		...
		public FileSystemResource download(@RequestParam("save_file_name")String save_file_name,@RequestParam("real_file_name")String real_file_name,HttpServletResponse response) throws Exception {
			//FileSystem...은 스프링 코어에서 제공하는 전용 파일처리 클래스 
			File file = new File(uploadPath + "/" + save_file_name);
			response.setContentType("application/download; utf-8");//아래한글,ppt문서 등 에서 한글내용이 깨지는 것을 방지하는 코드추가
			real_file_name = URLEncoder.encode(real_file_name);//ie에서 URL한글일때 에러발생방지 코드 추가
	...
	//파일 업로드 공통 메서드(Admin컨트롤러 에서 사용+Home컨트롤러 사용) 
