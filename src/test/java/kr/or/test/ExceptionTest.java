package kr.or.test;
/**
 * 이 클래스는 자바에서 개발자 예외를 처리하는 방법 실습하는 클래스입니다.
 * @author 김지애
 *
 */
public class ExceptionTest {
	...
	public static void main(String[] args) {
		...
		//동일 패키지안의 클래스는 import없이 사용 가능합니다.
		...
		for(int cnt=0;cnt<=2;cnt++) {
			/**
			 * 개발자가 예외처리하는 기본형식 try {(필수)구현프로그램에서 에러가 발생하면} 
			 * catch(에러발생상황) {에러발생시 처리할 구현내용} 
			 * finally {(선택사항)에러가 발생하든 아니든 무조건 실행}(선택사항)
			 */
			 try {
				indexValue = Integer.parseInt(stringArray[cnt]);	 
			 } catch(NumberFormatException ex) {//Exception 대신 선별적으로 예외사항을 잡습니다
				System.out.println(cnt + "숫자형태가 올바르지 않습니다, 확인해 주세요!");
