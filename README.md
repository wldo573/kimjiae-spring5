...
#### 20210603(목) 작업예정
- 작업비중(시간)=관리단 프로그램(70%)+사용(30%)
- 스프링에서 오라클연동 순서2가지:
- 1. jdbc(Java DabaBase Connection) 확장모듈 pom에 추가
- 2. 오라클 접속 드라이버 확장 모듈을 pom 
- root-context.xml 파일에 오라클 커넥션 빈(스프링클래스)을 추가.
- 스프링 관리하는 클래스를 추가하는 방법2-1:@Controller,@Repository @Service,@Component
- 스프링이 관리하는 클래스를 추가하는 방법2-2:xml에서 빈(bean태그)을 추가하면, 스프링 클래스됨
