...
- 사용자단 게시판 CRUD 처리.
- 사용자단 댓글 CRUD 처리.
- 헤로쿠 클라우드에 배포(9.클라우드배포CI/CD구현-개발트렌드).깃(최신소스)-연동-헤로쿠(배포)
- 문서작업(제출용)
...
- 코딩 테스트 10번 마무리 
- 코딩 테스트 9번 부터 시작,
- 코딩 테스트 8번 ,7번,6번까지 마무리.
- 8교시에 UI구현 워드문서 과제물 제출.제출전 7교시에 확인예정

####20210719(월)작업
- 로또번호가 올바른 번호인지 확인하는 코드 작성 코딩테스트 10번소스(아래) 작업중,,,
...
class Main {
	public static boolean isValid(int[] Lotto,int n){
		...
		if(n != 6){//조건1중 일부
			return false;//현재 isValid메서드가 종료되면서, false를 반환합니다.
		}
		//조건1 중복숫자 검사로직
		for(int i=0;i<(n-1);i++){
			if(Lotto[i] == Lotto[i+1]){
				return false;//중복숫자가 있으면,현재 isValid메서드를 종료하고,flase룰 반환합니다.
			}
		}
...
- 13 = 1101(2)
- 13 = 10의 자리1,1의 자리3
- 1101 = 8421(자리수)코드 =2(3)자리는1,2(2)자리는1,2(2)자리는1,2(1)자리는0,2(0)자리는1
- 모든수의 0승 (제곱)
...
class Main {
	public static void main(String[] args) {
		...
		int Mok, Nmg;//몫과 나머지의 변수로 사용
		Scanner sc = new Scanner(System.in);
		Dec = sc.nextInt();
		while(true){
			Mok = (int) Dec/2;
			Nmg = Dec - (Mok*2);//나머지룰 구하는 공식
			Bin[idx] = Nmg;
			idx = idx +1;//idx++
			if(Mok==0) {
				break;
...
class Main {
	public static void main(String[] args) {
	 int n;
		...
		System.out.prinyln("키보드로 입력받은 배열의 값은" + Arrays.toString(Numbers));
...
- 버블정렬 코딩테스트03소스(아래)오른차순에서
- 특징1:선택정렬은 제일작은 값이 왼쪽에 배치되면서 1회전이 종료
...
```
- Temp정렬 코딩 테스트02소스(아래).지난주에 사용한Arrays클래스 sort메서드구성연습
- 예,중복 for문에서 내부 for문 1회전-4회전
- 5, 4, 3, 2, 1(원시 데이터)
- 4, 5, 3, 2, 1(내부for회전)
- 3, 5, 4, 2, 1(내부for2회전)
- 2, 5, 4, 3, 1(내부for3회전)
- 1, 5, 4, 3, 2(내부for4회전)
- 외부 2회전 (1부터 시작 5보다 작을때까지)
- 1, 5, 4, 3, 2(원시데이터)
- 1, 4, 5, 3, 2(내부for1회전)
- 1, 3, 5, 4, 2(내부for2회전)
- 1, 2, 5, 4, 3(내부for3회전)
...
class Main {
	public static void main(String[] args) {
		...
		for(int i=0;i<n;i++){//키보드 정렬할 변수값을 입력 받습니다.Number[]배열변수에...
			Numbers[i] = sc.nextInt();
		}
		System.out.println("입력한 배열값은 " + Arrays.toSting(Numbers));
		//여기서부터 소칭로직 시작
		for(prev=0;prev<(n-1);prev++){//예,5개 숫자를 입력하면 4번번복합니다
			//n-1번만 이유는 4번까지 비교할 다음변수가 있시 때문에
			for(next=(prev+1);next<n;next++) {
				if(Numbers[perv] > Numbers[next]){//이전변수값이 더 크다면,앞 뒤 변수값을 자리 바꿈합니다
			...
			//System.out.println(prev +"번째 회전에서 나온 결과는"+Arrays.toString(Numbers));
		}
		//자리바꿈결과를 출력하는 구문 Arrays 유틸클래스 사용하지 않고 for문 사용
...
class Main {
	public static void main(String[] args) throws Exception {
		BufferedReader br = new BufferedReader(new InputStreamReader(System.in));//문자열을 생성하는 커서가 생김
		int UNIT = 50000;//화폐의 최고단위 금액,초기 화폐단위 초기화
		int NUM = 0;//(입력금액 / 단위금액) = 단위금액의 화폐매수 변수 초기화
		int SW = 0;//스위치변수 == 플래그(깃발)변수 초기화
		int MONEY = Integer.parseInt(br.readLine());
		while(true) {//IoT쪽에서는 while(true)문으로 외부데이터를 읽어 들입니다
			if(UNIT >1){
				NUM = (int) MONEY/UNIT;//화폐매수는 0.5매수는 없기 때문에 정수로 형변환합니다
				System.out.println(UNIT+"원의 화폐매수는 "+NUM);
				//다음반복을 위해서 MONEY변수 값 조정 UNIT변수값 조정
				MONEY = MONEY-(UNIT*5);//277777-250000 =25000(1회전결과)
				if(SW ==0 ){
					UNIT = UNIT/5;//5만원->1만원으로 단위변경(1회전결과,3회전결과,5회전결과)
					SW =1;//1회전 후 SW스위치변수값을 1로 변경
				}else{
					UNIT = UNIT/2;//5만원->1만원으로 단위변경(2회전결과,4회전결과,6회전결과)
					SW = 0;//1회전 후 SW스위치변수값을 1로 변경
			}else{
						break;//while반복문을 ㅃㅏ져나갑니다.무한반복을 벗어나는 코드
				}
...
```
-빅O 시간복잡도 구하기 :for문을 1개면, Big O=N번 횟수,
- 중복 for문이면, Big O = N^2
- for(i=1, i==3,i++){for(i=1,i==3,i++){구현로직}}
- 위 중복 for문은 시간복잡도가 O(N^2)번 횟수
- 프로그램의 성능을 측정하는 단위 빅O 표기법을 사용한다
- 화폐매수구하기:277777원 입금금액 있다면,
- 5만원짜리 지폐는 몇장인지, = 5장
- 1만원짜리 지폐는 몇장인지, = 2장
- 5천원짜리 지폐는 몇장인지, = 1장
- 1천원짜리 지폐는 몇장인지, = 2장
...
- SW=1 :1만, 1천원, 100원, 10원, 1원 = UNIT/2 끝(0.5원 화폐단위X)

####20210717(수)작업
- 네아로 로그인 부분 마무리.:외부API이고
- 요청URL생성 -> 성공여부(네이버로그인컨트롤러 메서드 추가)=> callback URL로 이동해서 처리하는 메서드 생성
- 문서작업(제출용)
...
- 서비스URL(사이트의 로그인URL)->네이버 로그인폼으로 진행(스프링 시큐리티로그인무시)
- 네이버 로그인폼에서 인증을 받으면(RestAPI에서 OAuth2.0인증)-> 서비스되는 사이트로 돌아오기(사이트URL필요 =@RequestMapping필요=콜백URL필요):시프링 시큐리티 로직을 타야합니다.
- 콜백메서드에서 하는 작업:enabled,ROLE_USER,session_값 지정을 할 수 있습니다.
- login_success는 스프링시큐리티의 인증 성공 후 이동할 URL위치를 구현 메서드
- naver_callback은 네이버OAuth2.0인증 성공후

####20210708(목)작업.
- 사용자단 게시물관리 CRUD중 Delete마무리 후 , Update실습
- 우리나라 스프링기반 솔루션을 만들던 시기(스프링 버전2.5 =2015년 전후)
  Rest-Api(jsonview방식)
  Rest-Api(@RestController방식,-ResponseBody)
- JsonView방식(고전방식의 RestAPI처리)실습
- 사용자단 메인페이지(대시보드)작업예정
- 헤로쿠 30분 지나서 휴먼모드로 들어가기전, 잠깨우는 기능 추가예정.(스프링스케줄러사용)
- 이력서 작업한 URL을 포토폴리오로 적어 놓으실때, 면접관이 1분정도 대기시간이 필요
- 헤로쿠클라우드는 처음 접속시 1 분정도 대기시간이 필요함(이력서에명시)

####20210707(수) 작업.
- 헤로쿠는 30분간 아무작업이 없으면 휴먼상태(컨테이너가 내려감) ->활성상태(컨테이너가 올라감)
- 컨테이너가 올라가면,클라우드 자원을 차지하기 때문,휴먼에서 활성화 될때 무료버전은 저장소가 신규생성됩니다
...
####20210701(목)작업
- 스프인시큐리티 로그인 및 권한체크 설정 후 사용자단 로그인/로그아웃 구현 예정(관리자단 끝 이면서, 사용자단 시작)
- 사용자단 회원가입,수정,탈퇴 JSP기능 추가예정
- 람다식 사용예 : https://github.com/miniplugin/SQLite-kimilguk/blob/master/app/src/main/java/com/human/sqlite_kimilguk/MainActivity.java
...
- 위 작업전, 크롬 확장프로그램중 부메랑을 한번 실습해 봅니다 id check 메서드로
- 네트워크로 자료를 담아서 보내는 방식-신버전
- Endpoint:마이크로 서비스,기존 컨트롤러로 RestAPI로 구현되고, 요청하는URL을 엔드포인트라고 합니다
- 엔드포인트(URL주소)에는 데이터를 전송할때 쿼리스트링으로 보내지 않는 방식 트랜드 
- 예, 구방식/reply/reply_list?bno=59&page=1 -> @RequestParam 애노테이션으로 받음
- 예, 신방식/reply/reply_list?/{게시물 변호값}/{페이지 변호값}->PathVariable
- 결과, /reply/reply_list/59/1(목적은 구글검색에 노출시키기 위해서)
- 트랜드 :마이크로서비스,기존 컨트롤러 (게이트웨이)를 모두RestCountroller(Rest게이트웨이)로 변경이 필요
...
- 댓글 VO제작-<매퍼쿼리제작->DAO클래스 제작->Service클래스 제작 ->@RestCountroller클래스제작->크롬 부메랑테스트(JUnit테스트대신)->JSP제작(1페이지CRUD처리 Ajax이용)
...
####20210623(수) 작업.
- 시큐어 코딩 방지메서드:<(O|o)bject...-> &It;object(목적은 코딩태그를 특수문자로 변경하는 메서드
...
####20210622(화)작업예정.
- 정합향으로 개발 시작 VO제작->매퍼쿼리제작-> DAO클래수제작->Service클래스제작->Controller+jsp
- 위 내용중 게시물 관리에서 CRUD 컨트롤러 + jsp처리(4.파일 업로드 구현)
- 관리자단 댓글관리 CRUD 처리(6.RestAPI서버구현,JUnit대신에 크롬부메랑으로 테스트)
...
- 관리자단 게시물관리 CRUD 처리(4.파일업로드구현,5.트랜잭션구현).
- @Service 클래스 마무리OK.
- 트랜잭션?여러개의 메서드를 1개처럼 처리하게 구현하는 애노테이션을 사용 
목적:데이터 무결성유지
- 1단어로 표시:All or NotAll(모두 실행되던지,에러발생 모두 실행이 되지 않던지)
- root-context와 servlet-context설정파일에 트랙잭션과 파일업로드 설정처리.
...
- 관리자단 게시물 관리CRUD처리(4.파일업로드구현,5.트랜잭션구현)
- 게시물관리 시작:다중게시판?1개 페이지로 board_type 변수를 이용해서 공지사항,

#### 20210617(목) 작업예정
- [복습]: 스프링의 기능 : IoC(제어의 역전:객체릐 메모리관리 개발자가 X, 스프링이 대신),DI(의존성 주입,@Inject)
- 관리자단 게시판 생성관리 RU 페이지 마무리예정
- 관리자단 게시판 생성관리 CD 처리.(3.스프링의AOP기능구현).
- 관리자단 외쪽메뉴에 게시판종류가 실시간으로 출력이 되야 하는데, 지금은 게시판 생성관리 목록 페이지에서만 보임.(문제점)
- 위 문제를 해결하는 방식으로 AOP기능을 사용합니다.
- 스프링 SapectOrientedProgram구현은 3가지 방식:@Aspect,@ControllerAdvice,intercept(가로채기)태그사용를 사용해서 관점지향프로그래밍을 구현.
- AOP용어: 관점지향?-프로그램전체에 영향을 주는 공통의 기능 적용하는 기법
- AOP용어: Advice(충고-간섭):프로세스작업 중간 필요한 기능을 끼워넣는 것을 어드바이스라고 함.
- Advice:포인트컷(간섭,필요한 기능을 끼워넣는 시점,@Before,@After,@Around실습)
- 게시판종류 출력:@ConteollerAdvice로 구현(게시판생성관리CRUD작업시 실습)
- 검색시 pageVO처럼 board_type을 값을 계속 유지하는 기능:@Aspect로 구현(게시물관리CRUD작업시 실습)
- @Aspect 장점:특정클래스의 특정메서드 실행시(포인트컷) 자동실행되는 메서드를 지정이 가능.
- @Aspect 실행조건:컨트롤러에 더해서 서비스(실습),DAO클래스의 메서드에도 Advice 가능.
- 로그인체크,권한체크시 : intercept(스프링시큐리티)태그를 사용해서 구현.(로그인기능,권한체크기능구현시 실습)
- 관리자단 게시판 생성관리 CD처리예정 (3.스프링의  AOP기능구현)
- UI디자인 과제물 제출 4교시OK
- 관리자단 게시판 생성관리 CRUD 처리.(4.파일업로드구현,5.트랜잭션구현).
- 게시물관리 시작:다중게시판? 1개 페이지로 board_type 변수를 이용해서 공지사항, 갤러리,QnA....구별해서 사용.
- 정방향으로 개발시작.VO제작->메퍼쿼리제작

#### 20210616(수) 작업예정
-10년,20년,지금 변하지 않는것은 변수값의 흐름은 변함이 없음.정방향 개발 시작
- ---------------------------------
-시작VO->매퍼쿼리->DAO클래스생성->Service클래스생성->컨트롤러 ->jsp화면처리
-관리자단 게시물관리 CRUD -처리(4.파일업로드구현,5.트랜잭션구현).
